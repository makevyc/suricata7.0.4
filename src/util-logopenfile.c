...
{
    //SCLogWarning("Connect Unix socket [%lu] \"%s\"", pthread_self(), path);
 ...
 * \retval 1 if it is now connected; otherwise 0
 */
static int SCLogUnixSocketReconnectEx(LogFileCtx *log_ctx, ThreadVars *tv)
{
    int disconnected = 0;
    if (tv->eve_fp) {
        SCLogWarning("Ex Write error on Unix socket \"%s\": %s; reconnecting...", log_ctx->filename,
                strerror(errno));
        fclose(tv->eve_fp);
        tv->eve_fp = NULL;
        tv->reconn_timer = 0;
    ...
    now = (uint64_t)tt.tv_sec * 1000;
    now += tt.tv_usec / 1000;           /* msec resolution */
    if (tv->reconn_timer != 0 &&
            (now - tv->reconn_timer) < LOGFILE_RECONN_MIN_TIME) {
        /* Don't bother to try reconnecting too often. */
        return 0;
    }
    tv->reconn_timer = now;

    tv->eve_fp = SCLogOpenUnixSocketFp(log_ctx->filename, log_ctx->sock_type, 0);
    if (tv->eve_fp) {
        /* Connected at last (or reconnected) */
        SCLogNotice("Ex Reconnected socket \"%s\"", log_ctx->filename);
    } else if (disconnected) {
        SCLogWarning("Ex Reconnect failed: %s (will keep trying)", strerror(errno));
    }

    return tv->eve_fp ? 1 : 0;  
}

static int SCLogFileWriteSocket(const char *buffer, int buffer_len,
        LogFileCtx *ctx, ThreadVars *tv)
{
    int tries = 0;
    int ret = 0;
    bool reopen = false;

    //if (ctx->fp == NULL && ctx->is_sock) {  // TODO 是否删除掉
    //    //SCLogUnixSocketReconnect(ctx);
    //}

    // 判断每个线程的sock是否需要重新连接
    if (tv && tv->eve_fp == NULL && ctx->is_sock) {
        SCLogUnixSocketReconnectEx(ctx, tv);
    }
...
tryagain:
    ...
    errno = 0;
    //if (ctx->fp != NULL) {
    if (tv && tv->eve_fp != NULL) {
        //int fd = fileno(ctx->fp);
        int fd = fileno(tv->eve_fp);
        ssize_t size = send(fd, buffer, buffer_len, ctx->send_flags);  // 0阻塞 MSG_DONTWAIT非阻塞
        if (size > -1) {
            ret = 0;
        } else {
    ...
    if (tv && reopen && tries++ == 0) {
        if (SCLogUnixSocketReconnectEx(ctx, tv)) {// TODO
            goto tryagain;  
        }
    }
 ...
 */
static int SCLogFileWriteNoLock(const char *buffer, int buffer_len, LogFileCtx *log_ctx, ThreadVars *tv)
{
    int ret = 0;
 ...
 * characters successfully written).
 */
static int SCLogFileWrite(const char *buffer, int buffer_len, LogFileCtx *log_ctx, ThreadVars *tv)
{
...
#ifdef BUILD_WITH_UNIXSOCKET
    if (log_ctx->is_sock) {
        ret = SCLogFileWriteSocket(buffer, buffer_len, log_ctx, tv);
    } else
#endif
    {
                     ...
                     int rotate)
{
    ...
    if (strcasecmp(filetype, "unix_stream") == 0) {
        snprintf(log_path, PATH_MAX, "%s", SURICATA_EVE_SOCK);
#ifdef BUILD_WITH_UNIXSOCKET
    ...
    SCLogInfo("log filename: %s", log_ctx->filename);
    ...
    if (log_ctx->is_sock && !IsRunModeOffline(RunmodeGetCurrent())) {
        ...
        //default send flags is MSG_DONTWAIT
        uint8_t send_flags = MSG_DONTWAIT;

        ConfNode* node = ConfGetNode("sr-custom.logging.unix-socket.send-mode");
        if (node != NULL) {
            int send_mode_enabled = 0;
            ConfGetChildValueBool(node, "enabled", &send_mode_enabled);
        
            if (send_mode_enabled) {
                const char* send_mode = ConfNodeLookupChildValue(node, "mode");
                if (send_mode != NULL && (strcasecmp(send_mode, "blocking") == 0)) {
                    send_flags = 0;
                }
            } 
        }

        if (send_flags & MSG_DONTWAIT) {
            SCLogInfo("Setting logging socket of non-blocking in live mode.");
        } else {
            SCLogInfo("Setting logging socket of blocking in live mode.");
        }
        log_ctx->send_flags = send_flags;
    }
#endif
...
int LogFileWrite(LogFileCtx *file_ctx, MemBuffer *buffer, ThreadVars *tv)
{
    if (file_ctx->type == LOGFILE_TYPE_FILE || file_ctx->type == LOGFILE_TYPE_UNIX_DGRAM ||
        ...
        file_ctx->Write((const char *)MEMBUFFER_BUFFER(buffer),
                        MEMBUFFER_OFFSET(buffer), file_ctx, tv);
