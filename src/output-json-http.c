...
static void EveHttpLogJSONBasic(JsonBuilder *js, htp_tx_t *tx)
{
    ...
    if (tx->request_hostname != NULL) {
        // Fix Bug19408: "hostname":"xtbg.gdzwfw.gov.cn, xtbg.gdzwfw.gov.cn"
        uint32_t req_hostname_len = bstr_len(tx->request_hostname);
        const unsigned char *data = bstr_ptr(tx->request_hostname);
        uint32_t valid_len = 0;
        while(valid_len < req_hostname_len && data && *data++ != ',') {
            valid_len++;
        }
        jb_set_string_from_bytes(
                js, "hostname", bstr_ptr(tx->request_hostname), valid_len);
    }
...
static void EveHttpLogJSONHeaders(
        JsonBuilder *js, uint32_t direction, htp_tx_t *tx, LogHttpFileCtx *http_ctx, const Packet *p)
{
    ...
    bool is_https_proto = false;
    if (p->livedev && strncmp(p->livedev->dev, "tls-tap", 7) == 0) {
        is_https_proto = true;
    }
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        // Fix content-encoding 内部已做解码 无需让前端解码，故不记录该字段
        if (strncasecmp(name, "content-encoding", sizeof("content-encoding")) == 0) {
            continue;
        }
        ...
        // Fix Host头出现重复 htp库的行为
        if (strncasecmp(name, "Host", sizeof("Host")) == 0) {
            uint32_t valid_len = 0;
            while(valid_len < size_value && value[valid_len] != ',') {
                valid_len++;
            }
            value[valid_len] = '\0';
        }
    ...
    if (array_empty) {
        ...
        // 模仿流量引擎日志，增加 :scheme表示协议类型用于审计日志展示
        jb_start_object(js);
        jb_set_string(js, "name", ":scheme");
        if (is_https_proto) {
            jb_set_string(js, "value", "https");
        } else {
            jb_set_string(js, "value", "http");
        }
        // close :scheme
        jb_close(js);
    ...
    if (htp_state) {
        ...
        if (tx) {
            ...
            if (htud != NULL) {
                ...
                if (json_output_ctx->replace_body_enabled && tx->response_content_type != NULL && json_output_ctx->content_type != NULL) {
                    ConfNode* tmp_node = NULL;
                    TAILQ_FOREACH(tmp_node, &json_output_ctx->content_type->head, next) {
                        if(tmp_node != NULL && (bstr_cmp_c_nocase(tx->response_content_type ,tmp_node->val) == 0)) {
                            is_match = true;
                            break;
                        }
                    }
                ...
                if (is_match) {
                    jb_set_string(js, "http_response_body_printable", json_output_ctx->replace_body);
                } else {
...
static void EveHttpLogJSON(JsonHttpLogThread *aft, JsonBuilder *js, htp_tx_t *tx, uint64_t tx_id,
                           const Packet *p, enum OutputJsonLogDirection dir)
{
    ...
    if (http_ctx->flags & LOG_HTTP_REQ_HEADERS || http_ctx->fields != 0)
        EveHttpLogJSONHeaders(js, LOG_HTTP_REQ_HEADERS, tx, http_ctx, p);
    if (http_ctx->flags & LOG_HTTP_RES_HEADERS || http_ctx->fields != 0)
        EveHttpLogJSONHeaders(js, LOG_HTTP_RES_HEADERS, tx, http_ctx, p);

    if (!tx->request_hostname && bstr_cmp_c(tx->request_protocol, HTTP_PROTO_1_0) == 0) {
        // HTTP1.0 host非必选，如不存在则使用目的IP做为hostname
        JsonAddrInfo addr_info = json_addr_info_zero;
        JsonAddrInfoInit(p, dir, &addr_info);
        jb_set_string(js, "hostname", addr_info.dst_ip);
    }
...
// http日志输出条件
static bool HttpLogCondition(const htp_tx_t *tx)
{
    // user-agent过滤
    if (tx->request_headers != NULL) {
        htp_header_t *h_user_agent = htp_table_get_c(tx->request_headers, "user-agent");
        if (h_user_agent) {
            if (bstr_cmp_c_nocase(h_user_agent->value, UA_ENVOY_HC) == 0 || 
                bstr_cmp_c_nocase(h_user_agent->value, UA_NUCLEI) == 0) {
                return false;
            }
        }
    }

    // 数据包异常：缺失http request包，request包在会话创建之前出现
    if (!tx->request_uri || bstr_cmp_c(tx->request_uri, URI_NOT_SEEN) == 0) {
        return false;
    }

    // protocol合法性校验 不以HTTP开头
    if (!tx->request_protocol || BSTR_NCMP_C(tx->request_protocol, HTTP_PROTO_PREFIX)) {
        return false;
    }

    // uri合法性校验
    if (BSTR_NCMP_C(tx->request_uri, HTTP_URI_PREFIX1) && BSTR_NCMP_C(tx->request_uri, HTTP_URI_PREFIX2)) {
        return false;
    }
...
static void SetHttpLogTime(Flow *f, JsonBuilder *js) {
    if (f == NULL) {
        return;
    }
    ...
    if (htp_state) {
        ...
        uint64_t req_time = SCTIME_MSECS(hstate->request_timestamp);
        uint64_t res_time = SCTIME_MSECS(hstate->response_timestamp);

        if (req_time && res_time) {
            if (res_time == req_time) {
                duration = 1; // 由于只精确到毫秒 所以如果是同一毫秒则向上取间隔时间为1秒
            } else {
                duration = res_time - req_time;
            }
        }
    }
    jb_set_uint(js, "duration", duration);
}
...
static int JsonHttpLogger(ThreadVars *tv, void *thread_data, const Packet *p, Flow *f, void *alstate, void *txptr, uint64_t tx_id)
{
    ...
    EveHttpLogJSON(jhl, js, tx, tx_id, p, LOG_DIR_FLOW);
    ...
    // 传递 ThreadVars *tv
    OutputJsonBuilderBuffer(js, jhl->ctx, tv);
    jb_free(js);
