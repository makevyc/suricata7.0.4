...
static void AlertJsonTunnel(const Packet *p, JsonBuilder *js)
{
    ...
    jb_set_uint(js, "tid", syscall(SYS_gettid));
    jb_close(js);
}
...
static void AlertAddAppLayer(const Packet *p, JsonBuilder *jb,
        const uint64_t tx_id, const AlertJsonOutputCtx* json_output_ctx)
{   
        ...
        case ALPROTO_HTTP1:
            ...
            if (EveHttpAddMetadata(p->flow, tx_id, jb)) {
                if (json_output_ctx->flags & LOG_JSON_HTTP_BODY) {
                    EveHttpLogJSONBodyPrintable(jb, p->flow, tx_id, json_output_ctx);
                }
...
static bool AlertLogCondition(const Packet *p, const PacketAlert *pa)
{
    if (!p->flow) {
        return true;
    }
    ...
    htp_tx_t *tx = AppLayerParserGetTx(IPPROTO_TCP, ALPROTO_HTTP1, htp_state, pa->tx_id);
    // request_uri_not_seen


    if (!tx || !tx->request_uri || bstr_cmp_c(tx->request_uri, URI_NOT_SEEN) == 0) {
        return false;
    }
...
static int AlertJson(ThreadVars *tv, JsonAlertLogThread *aft, const Packet *p)
{
    ...
    for (int i = 0; i < p->alerts.cnt; i++) {
        ...
        xff_buffer[0] = 0;
        if ((xff_cfg != NULL) && !(xff_cfg->flags & XFF_DISABLED) && p->flow != NULL) {  // disable
        ...
        if (p->flow != NULL) {
            if (json_output_ctx->flags & LOG_JSON_APP_LAYER) {
                AlertAddAppLayer(p, jb, pa->tx_id, json_output_ctx);
            }
            ...
            // if (json_output_ctx->flags & LOG_JSON_FLOW) {  //metadata
        ...
        OutputJsonBuilderBuffer(jb, aft->ctx, tv);;
        jb_free(jb);
    }
    ...
    //         OutputJsonBuilderBuffer(packetjs, aft->ctx, tv);
    //         jb_free(packetjs);
    //     }
    // }
...
static OutputInitResult JsonAlertLogInitCtxSub(ConfNode *conf, OutputCtx *parent_ctx)
{
    ...
    json_output_ctx->content_type = NULL;
    json_output_ctx->replace_body = NULL;
    json_output_ctx->replace_body_enabled = 0;

    ConfNode* body_type_node = ConfGetNode("sr-custom.http-body-type");
    if (body_type_node != NULL) {
        ConfGetChildValueBool(body_type_node, "enabled", &json_output_ctx->replace_body_enabled);
        if (json_output_ctx->replace_body_enabled) {
            json_output_ctx->replace_body = ConfNodeLookupChildValue(body_type_node, "body");
            if (json_output_ctx->replace_body == NULL) {
                json_output_ctx->replace_body = "useless data!";
            }
            json_output_ctx->content_type = ConfNodeLookupChild(body_type_node, "content-type");
            if(json_output_ctx->content_type == NULL) {
                SCLogInfo("Response body replacement has been enabled. But the content type that needs to be replaced is not set.");
            }
        }
    }
